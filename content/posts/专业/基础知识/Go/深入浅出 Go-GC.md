...
![image.png](https://img.xiaoshidui.top/blog-pic/images/20250921173754411.png)
...
Golang 的标记清扫算法的关键点就是标记这一步，其采用三色标记法，从 Root 对象开始进行可达性分析，关键步骤如下：
...
3. `gcMarkRootPrepare()` & `gcMarkTinyAllocs()` 这两步就是标记所谓的 Root 对象，可达性分析的初始：
   1. `gcMarkRootPrepare()` 主要是启动一些 Root 对象标记的工作线程，并不是直接进行标记，这些工作的线程数量和内存大小有关，例如：这里说的 Root 对象主要是.BSS 段和.DATA 段的对象，这些段大小，Go 底层会按照默认的块大小将他们分块，然后启动对应数量的协程到时候用于标记
...
首先 JVM 的 GC 是基于分代理论的，Java 设计团队认为，大多数对象都是朝生暮死，很多对象的生命周期都很短，所以将对象分为了 新生代、老年代以及永久代，固存储区域也就被氛围了新生代区、老年代区以及永久代区，其中新生代区又进一步划分为 Edan 区、S0 和 S1 区，其中的对象生命周期依次增长。<br>
...
2. 并发垃圾回收器
   ...
   3. ParNew 新生代采用标记-复制，其主要用于新生代，搭配 CMS 老年代回收器使用
...
3. G1 垃圾回收器
   ...
   4. 当堆内存使用量超过 45%，触发并发标记，标记后会根据暂停时间目标收集存活对象少的 Old 区
